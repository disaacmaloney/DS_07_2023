...
DROP PROCEDURE SP_LISTAR_NOTICIAS_FILTRO;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_LISTAR_NOTICIAS_FILTRO`(
    IN P_CAMPO VARCHAR(255),
    IN P_VALOR VARCHAR(255)
)
BEGIN
    SET @SQL = CONCAT("SELECT ID, TITULO, TEXTO, CATEGORIA, FECHA, IMAGEN ", 
                      "FROM NOTICIAS WHERE ", P_CAMPO , " LIKE '%" , P_VALOR ,"%'");
...
call SP_LISTAR_NOTICIAS_FILTRO('TITULO', 'A ')
